...
export const HoverCards: React.FC<HoverCardsBlockProps> = ({
  ...
  type HoverCardLink = NonNullable<
    NonNullable<HoverCardsBlockProps["cards"]>[number]
  >["link"];

  const getLinkUrl = (link: HoverCardLink) => {
  ...
  type HoverCardCard = NonNullable<
    NonNullable<HoverCardsBlockProps["cards"]>[number]
  >;

  const getImageUrl = (card: HoverCardCard) => {
