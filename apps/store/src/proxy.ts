...
async function getRegionMap(cacheId: string) {
  ...
  if (!BACKEND_URL) {
    throw new Error(
      "MEDUSA_BACKEND_URL environment variable is not configured. Set it in your .env file."
    );
  }
  ...
  if (
    ...
    try {
      if (!PUBLISHABLE_API_KEY) {
        throw new Error(
          "NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY environment variable is not configured. Set it in your .env file."
        );
      }
