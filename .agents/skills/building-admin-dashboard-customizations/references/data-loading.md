...
const response = await fetch(
  `http://localhost:9000/admin/products/${productId}`
);
...
```tsx
<Button onClick={handleSave} isLoading={updateProduct.isPending}>
  Save
</Button>
```
...
const { data } = useQuery({
  queryFn: () =>
    fetch("http://localhost:9000/admin/products").then((r) => r.json()),
  queryKey: ["products"],
});
